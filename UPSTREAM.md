...
| tosijs-ui | `IconElement` (the declared return type of every `icons.*`) is not re-exported from any entry point, so the 1.15 widening from `SVGElement` to the union is a breaking type change with no way to name the new type | [#176](https://github.com/tonioloewald/tosijs-ui/issues/176) | 🔵 open — filed 2026-09-21; worked around with a local alias in `src/toolbar.ts` that will silently drift when the union grows |
